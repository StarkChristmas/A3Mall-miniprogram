...
import { getRegimentDetail } from "../../../api/http";
...
Page({
  ...
  getCountDownStatus(event){
    this.setData({ 
      isActivityStatus: event.detail.status
    });
  },

  onBuy(){
    if(this.data.isActivityStatus == false){
      toast("活动己结束！");
      return false;
    }
  ...
  initGoodsInfo(){
    this.setData({
      ...
      selectedGoodsInfo : {
        ...
        goods_id        : this.data.products.goods_id,
