...
import { getGoodsDetail,goodsDetailAddCart,goodsDetailFavorite } from "../../../api/http";
...
Page({
  ...
  onFavorite(){
    ...
    goodsDetailFavorite({
      ...
      if(result.status){
        this.setData({
          collect: result.data == 1 ? true : false
        });
      }else{
        toast(result.info);
      }
  ...
  onCart(){
    ...
    goodsDetailAddCart({
      ...
      if(result.status){
        toast(result.info);
        this.setData({ cartCount: result.data.count });
        App.updateUserInfo({ shop_count: result.data.count });
      }else{
        toast(result.info);
      }
    }).catch((error)=>{
